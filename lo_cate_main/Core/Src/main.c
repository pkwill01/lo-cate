...
void lora_write_reg(uint8_t reg_addr, uint8_t reg_set, uint8_t *return_val) {
	HAL_StatusTypeDef hal_status;
	uint8_t tx_data[2] = { 0 };
	uint8_t rx_data[2] = { 0 };

	//define transmit and receive buffers
	tx_data[0] = reg_addr | 0x80;        // write operation work out why the fuck it needs | 0x80
	tx_data[1] = reg_set;                 // byte to write. 0x00 to read
...
void imu_read_reg(uint8_t reg_addr, uint8_t reg_set, uint8_t *return_val) {
	...
	//define transmit and receive buffers
	tx_data[0] = reg_set;                // read operation
	tx_data[1] = reg_addr;                 // byte to write. 0x00 to read
...
//	// LoRa module init
	...
	if(lora_initialise(918E6, lora_read_reg,lora_write_reg))
	{
		uint8_t buffer[] = "Lora init Success!\r\n";
		CDC_Transmit_FS(buffer, sizeof(buffer));
	}
	uint8_t tx_lora_buff[4] = {0x22,0x11,0x01,0x03};
	uint8_t rx_lora_buff[4] = {0};
	while (1) {
		/* USER CODE END WHILE */

		/* USER CODE BEGIN 3 */
//		lora_read_reg(0x42, 0x00, &rx_lora_buff);
		lora_transmit(tx_lora_buff,4);
		HAL_Delay(50);
//		lora_recive(rx_lora_buff);
//		HAL_Delay(500);
